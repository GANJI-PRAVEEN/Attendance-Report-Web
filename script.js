...
function applyDetails(classesAttended,TotalClasses,classesPerDay,attendancePercentage,targetDateString,availableClasses,xClasses){
    ...
    if(xClasses!=0 || availableClasses!=0){
      ...
      newpara.textContent=`Your atttendance report after ${availableClasses} Classes `;
      newpara.id="msgPara";
      ...
      currentAttendaceTag.textContent = `attendance after ${Math.ceil(availableClasses/classesPerDay).toFixed(0)} Days (${availableClasses}): ${attendancePercentage.toFixed(2)}%`;
    }
